...
class TestClientInstallParams(IntegrationTest):
    ...
    def test_client_install_authconfig_params(self):
        ...
        authconf = client.get_file_contents('/etc/sysconfig/authconfig',
                                            encoding='utf-8')
        exp_val_mkhomedir = "USEMKHOMEDIR=yes"
...
class TestClientInstallNegative(IntegrationTest):
    ...
    def test_client_force_parameter(self):
        ...
        try:
            ...
            # uninstall for the first time, should pass
            tasks.uninstall_client(self.clients[0], raiseonerr=True)

            # uninstall for the second time with force
            result = tasks.uninstall_client(self.clients[0],
                                            extra_args=['--force'],
                                            raiseonerr=False)
            exp_msg = "IPA client is not configured on this system"
            assert result.returncode != 0
            assert exp_msg.lower() in result.stderr_text.lower()
        finally:
            tasks.uninstall_client(self.clients[0],
    ...
    def test_client_install_hostname_localhost(self):
        ...
        try:
            ...
            result = tasks.install_client(self.master, client)
            assert result.returncode != 0
            assert exp_msg.lower() in result.stderr_text.lower()
